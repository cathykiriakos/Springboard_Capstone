...
Budget_Information <- read.csv(file.path(path,"Budget Information.csv") ) #Budget Data
...
Expense_Code_Lookup[,"Expense.Code"] <- as.character(Expense_Code_Lookup[,"Expense.Code"]) 
Activity_Code_Lookup[,"Activity.Code"] <- as.character(Activity_Code_Lookup[,"Activity.Code"]) 
Account_Code_Lookup[,"Account.Number"] <- as.character(Account_Code_Lookup[,"Account.Number"]) 

Expense_Code_Lookup <- data.frame(lapply(Code_Lookup,trimws) , stringsAsFactors = FALSE) 
...
###View Avantis Data frame to ensure merge call will funciton
```{r,View_Data_Avantis_Tranformed,include=FALSE}
head(Avantis_Transformed) 
```

###View GL Data frame to ensure merge call will funciton
...
This call allows for,easy identification of the expense code in the data.  It is accomplished  using all x or,GL Data to be merged with the respective expense code data frame
...
```{r, joinExpenseCode}
GL_Final <- left_join(GL_Final,Expense_Code_Lookup,
                  by = c("expense_code" = "Expense.Code"))
```
